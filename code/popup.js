...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderAll() {
    ...
    paths.forEach(function (path, idx) {
      ...
      // 3b. ▶ Open in current tab
      var runCurrentBtn = document.createElement('button');
	  runCurrentBtn.className = 'open-one';
      runCurrentBtn.textContent = '▶';
      runCurrentBtn.title       = 'Open in current tab';
      runCurrentBtn.addEventListener('click', function () {
        openPathInCurrentTab(path);
      });
      row.appendChild(runCurrentBtn);
  ...
  function openPathInCurrentTab(rawPath) {
    var path = normalizePath(rawPath);
